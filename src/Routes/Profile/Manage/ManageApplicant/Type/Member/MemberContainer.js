...
export default () => {
    ...
    const Delete = async e => {
        e.preventDefault();
        const currId = e.currentTarget.getAttribute("data-key");
        useMembers(prev => prev.filter(member => member.id !== currId));
        try {
            const {
                data: { deleteMember }
            } = await deleteClubMemberMutation({
                variables: {
        ...
        useMembers(prev => prev.filter(user => user.id !== currId));
    };
    return <MemberPresenter loading={getClubMemebersQuery.loading} members={members} onDelete={Delete} />;
};
