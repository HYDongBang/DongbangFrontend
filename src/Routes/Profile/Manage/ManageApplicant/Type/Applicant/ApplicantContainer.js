...
export default () => {
            ...
            applicants.push({
                ...
                form: questions.map((question, index) => {
                    return {
                        ...
                        answer: application.answer[index]
                    };
                }),
    ...
    const edit = async id => {
        if (applicants.filter(applicant => applicant.id === id)[0].status === "UNCHECKED") {
            try {
    ...
    return <ApplicantPresenter loading={getApplicantsQuery.loading} applicants={applicants} onCheck={onCheck} onAccept={onAccept} onReject={onReject} onDelete={onDelete} id={id} />;
};
